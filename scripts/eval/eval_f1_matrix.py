...
def eval_one(weights: str, data_yaml: str, split: str, imgsz: int,
    ...
    conf/iou must match scripts/evaluate.py, which produced the official
    numbers: it passes iou=0.7 while Ultralytics' own val() default is 0.6.
    Leaving it unset shifts mAP by about +0.01 — the same order as the
    effects being compared here.
    """
    from od_core.common import register_custom_modules
...
def main() -> int:
    ...
    ap.add_argument("--conf", type=float, default=0.001,
                    help="Matches scripts/evaluate.py")
    ap.add_argument("--iou", type=float, default=0.7,
                    help="NMS IoU; 0.7 matches scripts/evaluate.py, not the "
                         "Ultralytics val() default of 0.6")
    args = ap.parse_args()
